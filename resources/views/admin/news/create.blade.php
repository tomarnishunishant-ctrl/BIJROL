...
<head>
    ...
    <title>@if(isset($news)) Edit @else New @endif News | BIJROL Admin</title>
    ...
    <div class="admin-shell">
        ...
        <main class="admin-main">
            ...
            <div class="admin-card">
                <h1>@if(isset($news)) Edit @else New @endif Article</h1>

                <form method="POST" action="@if(isset($news)) {{ route('admin.news.update', $news) }} @else {{ route('admin.news.store') }} @endif" enctype="multipart/form-data">
                    @if(isset($news))
                        @method('PUT')
                    @endif
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="excerpt" name="excerpt" maxlength="500">{{ old('excerpt', $news->excerpt ?? '') }}</textarea>
                        <div class="form-text">Brief summary (optional).</div>
                    </div>

                    <div class="mb-3">
                        <label for="content" class="form-label">Content</label>
                        <textarea class="form-control" id="content" name="content">{{ old('content', $news->content ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @if(isset($news) && $news->image)
                            <div style="margin-top: 10px;">
                                <img src="{{ asset('storage/' . $news->image) }}" alt="Current image" style="max-height: 120px; border-radius: 6px; border: 1px solid var(--admin-line);">
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input" id="is_published" name="is_published" value="1" @if(old('is_published', $news->is_published ?? false)) checked @endif>
                                <label class="form-check-label" for="is_published">Publish immediately</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label for="published_at" class="form-label">Publish Date</label>
                            <input type="date" class="form-control" id="published_at" name="published_at" value="{{ old('published_at', isset($news->published_at) ? $news->published_at->format('Y-m-d') : '') }}">
                        </div>
                    </div>

                    <div style="display:flex; gap: 10px; flex-wrap: wrap;">
                        <button type="submit" class="btn btn-submit">Save Article</button>
                        <a href="{{ route('admin.news.index') }}" class="btn btn-cancel">Cancel</a>
                    </div>
                </form>
            </div>
        </main>
