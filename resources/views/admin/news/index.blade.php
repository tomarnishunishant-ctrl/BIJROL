...
<head>
    ...
    <title>Manage News | BIJROL Admin</title>
        ...
        <main class="admin-main">
            ...
            <div class="admin-card">
                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                    ...
                    <a href="{{ route('admin.news.create') }}" class="btn btn-success">+ Add New Article</a>
                </div>
                <p style="color: var(--admin-muted); margin-top: 6px;">Manage village news, announcements, and updates.</p>
            </div>
            ...
            <div class="admin-card" style="padding: 0; overflow: hidden;">
                            ...
                            <tbody>
                                @foreach($news as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->image)
                                                <img class="table-img" src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->title }}">
                                            @else
                                        ...
                                        <td>{{ \Illuminate\Support\Str::limit($item->excerpt, 80) }}</td>
                                        <td>
                                        ...
                                        <td>{{ $item->published_at ? $item->published_at->format('M d, Y') : '-' }}</td>
                                        <td>
                                            <div style="display:flex; gap:6px; flex-wrap:wrap;">
                                                <a href="{{ route('admin.news.edit', $item) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                                                <form method="POST" action="{{ route('admin.news.destroy', $item) }}" onsubmit="return confirm('Delete this article?');" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                ...
                @else
                    <div class="empty-state">No news articles yet. Click "Add New Article" to create one.</div>
                @endif
            </div>
        </main>
    </div>
